 ...
 *   - `provider`     test that the provider string matches
 *
 * Examples:
 *
 *     app.get('/profile',
...
module.exports = function ensureLoggedIn(options) {
  ...
  return function(req, res, next) {
    var isProviderAllowed = (!!provider || (!!req.user && (provider == req.user.provider)));
    if (!req.isAuthenticated || !req.isAuthenticated() || !isProviderAllowed) {
